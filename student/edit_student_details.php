		...
		if ($result->num_rows == 0) {
            if($result1->num_rows == 0){
                ...
                if($img_size === 0)
                {
                    // echo "<script type='text/javascript'>alert('$img_size')</script>";
                    // $xx = $_FILES['image'];
                    $sql1="UPDATE users SET  name = '$name', email = '$email',username = '$username', role = '$role', password='$pswd' where Id='$user_id'";
                    ...
                    if ($error === 0) {
                        if ($img_size > 625000) {
                            ...
                            if (in_array($img_ex_lc, $allowed_exs)) {
                                ...
                                echo "<script type='text/javascript'>alert('$em')</script>";
                                header( "Refresh:0.01; url=index.php?error=$em", true, 303);
                            }
                        }
                    }else {
            ...
            else
            {
                ...
                header( "Refresh:0.01; url=index.php?error=Username not available.", true, 303);
            }
        }
        else
        {
                echo "<script type='text/javascript'>alert('email address already used')</script>";
                header( "Refresh:0.01; url=index.php?error=email address already used", true, 303);
        }
